...
function BrowseQuizzes() {
	...
	const fetchNextQuiz = useCallback(
		async ({ filtersOverride, skipLoading = false } = {}) => {
			if (!skipLoading) {
				setLoading(true);
			}
			try {
				const activeFilters = filtersOverride ?? currentFilters;
	...
	useEffect(() => {
		const initializeData = async () => {
			await Promise.all([
				fetchStats(),
				fetchNextQuiz({ skipLoading: true }),
			]);
			setInitialLoad(false);
		};
		initializeData();
	}, [fetchStats, fetchNextQuiz]);
	...
	const handleFilterChange = async (e) => {
		...
		await fetchNextQuiz({ filtersOverride: newFilters });
	};
	...
	const resetFilters = async () => {
		...
		fetchNextQuiz({ filtersOverride: clearedFilters });
	};
